...
class Image_GD_Driver extends Image_Driver
{
    ...
    public function resize($properties)
    {
        ...
        //---------------
        $thumb_width = $properties['width'];
        $thumb_height = $properties['width'];
        ...
        if (floor($thumb_width - $properties['width'])>0) {
            $left    =floor(($thumb_width - $properties['width']) / 2);
        }

        if (floor($thumb_height - $properties['height'])>0){
            $top     = floor(($thumb_height - $properties['height']) / 2);

        }


        // Execute the resize
        if ($status = imagecopyresampled($img, $this->tmp_image, $left, $top, 0, 0, $properties['width'], $properties['height'], $width, $height)) {
            // Swap the new image for the old one
            imagedestroy($this->tmp_image);
            $this->tmp_image = $img;
        }
    ...
    public function watermark($params)
    {
        ...
        $w_x = (imagesx($this->tmp_image)-imagesx($mark))-$x;
        $w_y = (imagesy($this->tmp_image)-imagesy($mark))-$y;




        imagecopyresized($this->tmp_image, $mark, $w_x, $w_y, 0, 0, imagesx($mark), imagesy($mark), imagesx($mark), imagesy($mark));
        imagedestroy($mark);
