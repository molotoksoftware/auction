...
//	jQuery('.scroll-pane').jScrollPane();
//	jQuery('div.select_search_cat .sbOptions').jScrollPane();



/*	$("#town_id").selectbox({
		onOpen: function (inst) {
			$(this).next('.sbHolder').addClass('active');
		},
		onClose: function (inst) {
			$(this).next('.sbHolder').removeClass('active');
		}
	});
...
//	});
	...
	*/
    ...
    // Перекидываем на информационную страницу при нажатии на звезду
    $('.star_ver').click(function(){
        location.href='/pages/rating';
    });

    // Прокрутка наверх при клике на погинатор грида.
 /*   $('.yiiPager > li > a').live('mouseup', function () {
        $('html, body').animate({ scrollTop: 0 }, 150);
    }); */

    // Чекбокс "Выбрать все" для гридов.
